...
function back() {
	if (!analysisPending) {
		...
		if (moveHistory[moveHistory.length - 1]) {
			var lastBack = moveHistory.pop()
			backedMoves.push(lastBack)
			setLineToDelete(lastBack)
		}
	}
}
...
function importGames() {
	...
	if (!analysisPending) {
		...
		importPgn(importParameters.openingDepth, importParameters.depth).then(response => {
			serverReady(); analysisPending = false;
			enableAnalysisButtons()
